...
class StoryMenuState extends MusicBeatState
{
	...
	private static var curWeek:Int = 7;
	...
	function selectWeek()
	{
		if (curWeek >= weekUnlocked.length || weekUnlocked[curWeek])
		{
			switch (curWeek)
			{
				...
				case 6:
					if (WeekData.songsNames[curWeek][0] != '')
					{
					...
					else if (MASKstate.getProgress() > 0 && FlxG.save.data.p_progress[4] == 0)
	...
	function updateText()
	{
		...
		if (curWeek == 6 && !FlxG.save.data.ending[2])
			stringThing[0] = '';
