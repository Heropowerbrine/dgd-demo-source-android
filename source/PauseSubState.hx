...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		var cj = [0, 0, 13, 13, 20, 20];
		var controlArray = ClientPrefs.lastControls.copy();
		for (i in 0...Main.ammo[PlayState.mania])
		{
			if (PlayState.mania == 2 && i == 3) cj[2] ++;
			if (PlayState.mania == 4 && i == 4) cj[4] ++;
			if (PlayState.mania == 1 && i == 2) cj[1] += 10; //fix 5k bullshit
			if (PlayState.mania == 1 && i == 3) cj[1] -= 11;
