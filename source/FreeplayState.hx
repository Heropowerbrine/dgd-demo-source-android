...
class FreeplayState extends MusicBeatState
{
	...
	function changeDiff(change:Int = 0)
	{
		...
		var amount:Int = 2;
		if (songs[curSelected].songName.toLowerCase() == 'desecration')
			amount = 1; //allows you to see old when hovering over des

		if (curDifficulty < 0)
			curDifficulty = CoolUtil.difficultyStuff.length - amount;
		if (curDifficulty >= CoolUtil.difficultyStuff.length - (amount-1))
			curDifficulty = 0;

		#if !switch
		intendedScore = Highscore.getScore(songs[curSelected].songName, curDifficulty);
		intendedRating = Highscore.getRating(songs[curSelected].songName, curDifficulty);
		if (songs[curSelected].songName.toLowerCase() == 'desecration' && curDifficulty == 3)
		{
			intendedScore = Highscore.getScore(songs[curSelected].songName + '-old', curDifficulty); //fix score
			intendedRating = Highscore.getRating(songs[curSelected].songName + '-old', curDifficulty);
		}
		#end

		PlayState.storyDifficulty = curDifficulty;
		var diffTxt = ['MANIA', 'EASY', 'CANON', 'OLD'];
