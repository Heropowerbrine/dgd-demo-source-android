...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		...
		menuBG = new FlxSprite().loadGraphic(Paths.image('optionsmenu'));
...
class ControlsSubstate extends MusicBeatSubstate {
	...
	var optionShit:Array<String> = [
		...
		'5 KEY SPACE',
		ClientPrefs.keyBinds[12][1], //fuck you psych
		'6 OR 7 KEY',
...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Bar Noteskin':
						ClientPrefs.barNotes = !ClientPrefs.barNotes;

					case 'Note Hit Movements':
						ClientPrefs.noteHitMovements = !ClientPrefs.noteHitMovements;
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					...
					/*case 'Lane Trasparency':
						var mult:Float = 0.1;
						if(holdTime > 1.5) { //Double speed after 1.5 seconds holding
							mult = 0.2;
						}
						ClientPrefs.laneAlpha += add * mult;
						ClientPrefs.laneAlpha = FlxMath.roundDecimal(ClientPrefs.laneAlpha, 1); //round to prevent floating point weirdness
						if(ClientPrefs.laneAlpha < 0) ClientPrefs.laneAlpha = 0;
						else if(ClientPrefs.laneAlpha > 1) ClientPrefs.laneAlpha = 1;*/
				}
				reloadValues();
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Bar Noteskin': 
				daText = "If checked, notes will be bars rather than arrows.";
			case 'Note Hit Movements':
				daText = "If checked, the camera will move to the direction\nof the note.";
			case 'Flashing Lights':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Bar Noteskin': 
						daValue = ClientPrefs.barNotes;
					case 'Note Hit Movements':
						daValue = ClientPrefs.noteHitMovements;
					case 'Flashing Lights':
