...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Even Blue is disappointed', 0.2], //From 0% to 19%
        ['Insert shaggy quote that degrades the player here', 0.4], //From 20% to 39%
        ['WB is disappointed', 0.5], //From 40% to 49%
        ['Try harder', 0.6], //From 50% to 59%
        ['Not bad', 0.69], //From 60% to 68%
        ['Doing great', 0.7], //69%
        ['Shaggy is impressed', 0.8], //From 70% to 79%
        ['Zoinks!!', 0.9], //From 80% to 89%
        ['How?', 1], //From 90% to 99%
	...
	public var backshit:FlxBackdrop = null;
	...
	override public function create()
	{
		...
		{
			case 'where-are-you' | 'eruption' | 'kaio-ken' | 'whats-new' | 'blast' | 'super-saiyan' | 'overflow'|'shotgun-shell':
				defaultCamZoom = 0.9;
				curStage = 'mansion';
			...
			case 'god-eater':
				...
				*/
				...
				legs.alpha = 0;
			case 'astral-calamity' | 'talladega'|'conqueror':
				defaultCamZoom = 0.56;
				if (SONG.song == 'Talladega') defaultCamZoom = 0.6;

				if (SONG.song.toLowerCase() == 'conqueror')
				{
					
					//backshit = new FlxBackdrop(camGame.buffer); //messing with shit idk
					//backshit.cameras = [camHUD];

					/*camGame.width = Std.int(1280 * 0.95);
					camGame.height = Std.int(720 * 0.95);
					camGame.bgColor.alpha = 0;
					camGame.useBgAlphaBlending = true;*/
			...
			case 'soothing-power' | 'thunderstorm' | 'dissasembler':
				...
				add(ground);
			case 'electroblast' | 'desecration'| 'desecration-old':

				defaultCamZoom = 0.55;
				curStage = 'rooftop';

				var sky:BGElement = new BGElement('BlueShaggy/Blue Shaggy Sky', -530, -350, 0.05, 1.2, 3);
				add(sky);
				
				var floor:BGElement = new BGElement('BlueShaggy/Blue Shaggy Floor', -650, -680, 1, 1.2, 2);
				add(floor);
								
				var forewalls:BGElement = new BGElement('BlueShaggy/Blue Shaggy Foreground Walls', -800, -845, 1.1, 1.4, 1);
				//forewalls.scrollFactor.y = 0.8;
				var sidewalls:BGElement = new BGElement('BlueShaggy/Blue Shaggy Walls', -750, -400, 1, 1.3, 0);

				foreList.push(sidewalls);
				foreList.push(forewalls); //forewalls in front
			...
			case 'weed': 
				...
				var sky:BGElement = new BGElement('Weed/Weed sky', -333, -469, 0.1, 1.2, 0);
				add(sky);

				var moon:BGElement = new BGElement('Weed/Weed moon', -417, -392, 0.1, 1.2, 1);
				add(moon);

				var bg:BGElement = new BGElement('Weed/Weed background', -299, -448, 1, 1.2, 2);
				add(bg);

				var fg:BGElement = new BGElement('Weed/Weed foreground', -299, -448, 1.1, 1.2, 3);
				//foreList.push(fg);
				add(fg);
		...
		{
			case 'rooftop': 
				boyfriend.cameraPosition[1] -= 200;
			case 'cave': 
				boyfriend.cameraPosition[1] -= 170;
				camPos.y -= 170;
		}
		for (i in foreList)
			foregroundGroup.add(i);

		foreList = []; //not used after this point
		...
		songPopup.visible = !ClientPrefs.hideHud;
		add(songPopup);
		...
		switch(SONG.song.toLowerCase())
		{
			case "where-are-you" | "eruption" | "kaio-ken" | "whats-new" | "blast" | "super-saiyan" | "god-eater" | "soothing-power" | "astral-calamity" | "talladega":
				composer = "srPerez";
			case "thunderstorm": 
				composer = "Saruky";
			case "dissasembler":
				composer = "Joan Atlas";
			case "conqueror" | "weed" | "ultra-instinct": 
				composer = "Totally-Not-Genji";
			case "desecration" | 'desecration-old': 
				composer = "Spurk";
			case "electroblast": 
				composer = "RaymondVito";
		}
		...
		popupText.visible = !ClientPrefs.hideHud;
	...
	function zephReturns()
	{
		dad.alpha = 0;
		camFollow.x = gf.getMidpoint().x;
		camFollow.y = gf.getMidpoint().y;
		iconP2.alpha = 0;
		new FlxTimer().start(3, function(cock:FlxTimer)
		{
			
			camFollow.x = dad.getMidpoint().x;
			camFollow.y = dad.getMidpoint().y;
			new FlxTimer().start(1, function(shit:FlxTimer)
			{
				dad.alpha = 1;
				iconP2.alpha = 1;
				new FlxTimer().start(1, function(fuck:FlxTimer)
				{
					schoolIntro(0);
	...
	var burstFrames:FlxFramesCollection = null;

	public function burstRelease(bX:Float, bY:Float, playSound:Bool = true)
	{
		FlxG.sound.play(Paths.sound('burst'), playSound ? 1 : 0);
		remove(burst);	
		burst = new FlxSprite(bX - 1000, bY - 100);
		if (burstFrames == null)
			burstFrames = Paths.getSparrowAtlas('characters/shaggy');
	...
	function eventPushed(event:Array<Dynamic>) {
		switch(event[2]) {
			...
			case 'Shaggy burst': 
				if (!preloadedBurst)
				{
					FlxG.bitmap.add(Paths.image('characters/shaggy')); //burst is on green shag spritesheet so this just loads into memory for less lag
					/*var temp:Character = new Character(0,0, 'shaggy'); 
					temp.alpha = 0.001;
					add(temp);
					remove(temp);*/
					CoolUtil.precacheSound('burst');
					burst = new FlxSprite(0, 0);
					if (burstFrames == null)
						burstFrames = Paths.getSparrowAtlas('characters/shaggy');
					burst.frames = burstFrames;
					burst.animation.addByPrefix('burst', "burst", 30);
		...
		for (i in 0...Main.ammo[mania])
		{
			...
			switch (curStage)
			{
				...
				default:
					...
					if (ClientPrefs.barNotes)
					{
						dirName = Main.gfxDir[Main.gfxAlterInd[mania][i]];
						pressName = Main.gfxLetter[Main.gfxAlterInd[mania][i]];
					}
			...
			if (!isStoryMode)
			{
				...
				if (SONG.song.toLowerCase() != 'weed' || player == 1)
				{
					babyArrow.y -= 10;
					babyArrow.alpha = 0;
			...
			if (SONG.song.toLowerCase() == 'weed' && player == 0)
			{
				if (ClientPrefs.downScroll)
				{
					FlxTween.tween(babyArrow, {y: -300}, 1, {ease: FlxEase.cubeInOut, startDelay: 0.5 + (0.2 * i)});
				}
				else
				{
					FlxTween.tween(babyArrow, {y: FlxG.height + 300}, 1, {ease: FlxEase.cubeInOut, startDelay: 0.5 + (0.2 * i)});
				}	
			}
	...
	public static var rotNoteCam = false;
	var rotNoteCamSpd:Float = 1;
	var rotNoteCamRange:Float = 10;
	var rotNoteCamInd = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		{
			rotNoteCamInd ++;
			camHUD.angle = Math.sin(rotNoteCamInd / 100 * rotNoteCamSpd) * rotNoteCamRange;
		}
		else
		{
			rotNoteCamInd = 0;
			...
			case 1:
				...
				switch (wb_time)
				{
					...
					case 1300:
						...
						MusicBeatState.switchState(new CreditsState(38));
				}
		}
		switch (zend_state)
		{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.ignoreNote)
				{
					...
					doNoteCamMove(dad, daNote.noteData);
					dad.holdTimer = 0;
									...
									default:
										...
										if(ClientPrefs.ghostTapping) {
											if (boyfriend.curCharacter == 'bf')
												boyfriend.playAnim('sing' + Main.charDir[Main.gfxHudBF[mania][Std.int(Math.abs(daNote.noteData))]] + 'miss', true);
											else 
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, ?onLua:Bool = false) {
		switch(eventName) {
			...
			case 'HUD Camera rotate on':
				rotNoteCam = true;
				rotNoteCamSpd = Std.parseFloat(value1);
				rotNoteCamRange = Std.parseFloat(value2);
			case 'HUD Camera rotate off':
				rotNoteCam = false;
				camHUD.angle = 0;
			case 'Toggle bg dim':
				dimGo = !dimGo;
			case 'Change Cam Speed':
				camLerp = Std.parseFloat(value1);
			case 'Drop eye':
	...
	public function moveCamera(isDad:Bool) {
		if(isDad) {
			...
			camFollow.x += dad.cameraPosition[0] + dad.camNoteHitOffset[0];
			camFollow.y += dad.cameraPosition[1] + dad.camNoteHitOffset[1];
			...
			camFollow.x -= boyfriend.cameraPosition[0] - boyfriend.camNoteHitOffset[0];
			camFollow.y += boyfriend.cameraPosition[1] + boyfriend.camNoteHitOffset[1];
	...
	function doNoteCamMove(character:Character, dir:Int):Void
	{
		if (!ClientPrefs.noteHitMovements)
			return; 

		switch (Main.charDir[Main.gfxHudBF[mania][Std.int(Math.abs(dir))]])
		{
			case 'LEFT': 
				//if (character.camNoteHitOffset[0] >= -100)
					//character.camNoteHitOffset[0] -= 25;
				character.camNoteHitOffset = [0,0]; //reset it
				character.camNoteHitOffset[0] = -character.height / 20; //use height for all so its the same val for each dir
			case 'RIGHT': 
				character.camNoteHitOffset = [0,0];
				character.camNoteHitOffset[0] = character.height / 20;
			case 'UP': 
				character.camNoteHitOffset = [0,0];
				character.camNoteHitOffset[1] = -character.height / 20;
			case 'DOWN':
				character.camNoteHitOffset = [0,0];
				character.camNoteHitOffset[1] = character.height / 20;
			case 'SPACE': 
	...
	function noteMiss(direction:Int = 1):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (boyfriend.curCharacter == 'bf')
				boyfriend.playAnim('sing' + Main.charDir[Main.gfxHudBF[mania][direction]] + 'miss', true);
			else
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				...
				doNoteCamMove(boyfriend, note.noteData);
			}
			...
			if(note.noteType == 2) {
				...
				if (boyfriend.curCharacter == 'bf')
					animToPlay = 'sing' + Main.charDir[Main.gfxHudBF[mania][Std.int(Math.abs(note.noteData))]];
				else
